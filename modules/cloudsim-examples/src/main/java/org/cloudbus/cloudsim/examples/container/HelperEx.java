...
//             data.append(String.format("%.5f", slaTimePerVmWithMigration) + delimeter);
            ...
            Log.printLine(String.format("Energy consumption: %.4f kWh", energy));
            Log.printLine(String.format("Number of VM migrations: %d", numberOfVmMigrations));
    ...
    public static void printResultsNewWL(PowerContainerDatacenter datacenter,
        ...
        double sumNumberOfContainerMigrations = Double.NaN;
        if (!numberOfContainerMigrations.isEmpty()) {
            sumNumberOfContainerMigrations=MathUtil.sum(numberOfContainerMigrations);
            meanNumberOfContainerMigrations = MathUtil.mean(numberOfContainerMigrations);
            //standard deviation（σ/标准偏差）
...
//            CSVWriter writer1 = new CSVWriter(new FileWriter(fileAddress, true), ',',CSVWriter.NO_QUOTE_CHARACTER);
            ...
            Log.printLine(String.format("Energy consumption: %.2f kWh", energy));
            Log.printLine(String.format("Energy consumption: %.4f kWh", energy));
            Log.printLine(String.format("Number of sumNumberOfContainerMigrations: %.2f", sumNumberOfContainerMigrations));
            Log.printLine(String.format("Number of meanNumberOfContainerMigrations: %.2f", meanNumberOfContainerMigrations));
            Log.printLine(String.format("Number of stDevNumberOfContainerMigrations: %.2f", stDevNumberOfContainerMigrations));
            Log.printLine(String.format("Number of medNumberOfContainerMigrations: %.2f", medNumberOfContainerMigrations));
            Log.printLine(String.format("SLA: %.5f%%", sla * 100));
            Log.printLine(String.format(
                    "SLA perf degradation due to migration: %.2f%%",
                    slaDegradationDueToMigration * 100));
            ...
            Log.printLine(String.format(
                    ...
                    stDevTimeBeforeHostShutdown));
            Log.printLine(String.format(
                    "med time before a host shutdown: %.2f sec",
                    medTimeBeforeHostShutdown));
//            Log.printLine(String.format(
//                    "Mean time before a VM migration: %.2f sec",
//                    meanTimeBeforeVmMigration));
...
//                    stDevTimeBeforeVmMigration));
            ...
            Log.printLine(String.format(
                    ...
                    stDevTimeBeforeContainerMigration));
            Log.printLine(String.format(
                    "med time before a Container migration: %.2f sec",
                    medTimeBeforeContainerMigration));

            /**
             * 自己添加
             */
            Log.printLine(String.format("meanActiveVm: %.2f",meanActiveVm));
            Log.printLine(String.format("stDevActiveVm: %.2f",stDevActiveVm));
            Log.printLine(String.format("medActiveVm: %.2f",medActiveVm));
            Log.printLine(String.format("meanActiveHosts: %.2f",meanActiveHosts));
            Log.printLine(String.format("stDevActiveHosts: %.2f",stDevActiveHosts));
            Log.printLine(String.format("medActiveHosts: %.2f",medActiveHosts));
            Log.printLine(String.format("meanNumberOfContainerMigrations: %.2f",meanNumberOfContainerMigrations));
            Log.printLine(String.format("stDevNumberOfContainerMigrations: %.2f",stDevNumberOfContainerMigrations));
            Log.printLine(String.format("medNumberOfContainerMigrations: %.2f",medNumberOfContainerMigrations));
            Log.printLine(String.format("meanDatacenterEnergy: %.2f",meanDatacenterEnergy));
            Log.printLine(String.format("stDevDatacenterEnergy: %.2f",stDevDatacenterEnergy));
            Log.printLine(String.format("medDatacenterEnergy: %.2f",medDatacenterEnergy));
            Log.printLine(String.format("totalContainerMigration: %d",totalContainerMigration));
            Log.printLine(String.format("totalVmMigration: %d",totalVmMigration));
            Log.printLine(String.format("totalVmCreated: %d",totalVmCreated));
            Log.printLine(String.format("numberOfOverUtilization: %d",numberOfOverUtilization));
            Log.printLine(String.format("ContainersCreated: %d",broker.getContainersCreated()));
            Log.printLine(String.format("NumberOfCreatedVMs: %d",broker.getNumberOfCreatedVMs()));
