...
//        }
    ...
    public List<Double> getCurrentRequestedMips() {
        ...
        if (isBeingInstantiated()) {
            //System.out.println("VMallocation-5-3-1-A");
            requestedMipsTemp = getMips();
        } else {
            System.out.println("VMallocation-5-3-1-B");
            for (Container contianer : getContainerList()) {

                System.out.println("contianerPE:"+contianer.getNumberOfPes()+" "+contianer.getCurrentRequestedMips().get(0));
...
//            Log.formatLine("Total mips usage is %.2f", requestedMipsTemp);
        ...
        for (int i = 0; i < getNumberOfPes(); i++) {
            ...
            //System.out.print("requestedMipsTemp:"+requestedMipsTemp);
        }
        //System.out.println();
    ...
    public double getCurrentRequestedTotalMips() {
        //System.out.println("VMallocation-5-3-ContainerVm.getCurrentRequestedTotalMips");
        double totalRequestedMips = 0;
        //计算CurrentRequestedMips的各元素总和
    ...
    public double getCurrentRequestedMaxMips() {
        //System.out.println("VMallocation-5-2-ContainerVm.getCurrentRequestedMaxMips");
        double maxMips = 0;
        for (double mips : getCurrentRequestedMips()) {
            if (mips > maxMips) {
                maxMips = mips;
    ...
    public double getTotalUtilizationOfCpuMips(double time) {
        ...
        System.out.println("VMmigration-10-0-1-ContainerVm.getTotalUtilizationOfCpuMips");
...
//        Log.printLine("ContainerVm: re alocating MigratingInContainer:......");
    ...
    public boolean isSuitableForContainer(Container container) {
        //System.out.println("Containerallocation-1-3-1-ContainerVm.isSuitableForContainer");
...
//                .isSuitableForContainer(container, container.getCurrentRequestedBw()));
        //自己
        double a=getContainerScheduler().getAvailableMips();
        double b=container.getWorkloadTotalMips();
        double c=container.getCurrentRequestedTotalMips();
        List<Double> d=container.getCurrentRequestedMips();
        //经对比getCurrentRequestedTotalMips和getWorkloadTotalMips的大小关系不明确，大多数是前者大于等于后者
        //System.out.println("WorkloadTotalMips"+b+"CurrentRequestedTotalMips"+c+"CurrentRequestedMips"+d);
//        return (getContainerScheduler().getAvailableMips() >= container.getWorkloadTotalMips()
//                && getContainerRamProvisioner().isSuitableForContainer(container, container.getCurrentRequestedRam()) && getContainerBwProvisioner()
//                .isSuitableForContainer(container, container.getCurrentRequestedBw()));
        if (!getContainerRamProvisioner().isSuitableForContainer(container, container.getCurrentRequestedRam())){
            System.out.println("Vm:"+this.getId()+",Ram:"+this.getCurrentAllocatedRam()+"RamIsNotSuitableForContainer"+container.getId()+",Ram:"+container.getCurrentRequestedRam());
        }
        if (!getContainerBwProvisioner().isSuitableForContainer(container, container.getCurrentRequestedBw())){
            System.out.println("Vm:"+this.getId()+"BwIsNotSuitableForContainer"+container.getId());
        }
        return (a >= c && getContainerRamProvisioner().isSuitableForContainer(container, container.getCurrentRequestedRam())
...
//        Log.printLine("ContainerVm: get the total mips......" + ContainerPeList.getTotalMips(getPeList()));
    ...
    public double getAvailableMips() {
        ...
        //System.out.println("VMmigration-10-1-1-ContainerVm.getAvailableMips");
...
//        Log.printLine("Host: Create VM???......" + container.getId());
        System.out.println("Containerallocation-1-2-1-ContainerVm.containerCreate");
        if (getSize() < container.getSize()) {
        ...
        /**
         * @see org.cloudbus.cloudsim.container.schedulers.ContainerSchedulerTimeSharedOverSubscription#allocatePesForContainer
         */
        List<Double> currentRequestedMips = new ArrayList<Double>();
        currentRequestedMips.addAll(container.getCurrentRequestedMips());
        //System.out.println("ContainerCurrentRequestedMips:"+currentRequestedMips);
        //if (!getContainerScheduler().allocatePesForContainer(container, container.getCurrentRequestedMips())) {
        if (!getContainerScheduler().allocatePesForContainer(container, currentRequestedMips)) {
                Log.printConcatLine("[ContainerScheduler.ContainerCreate] Allocation of Container #", container.getId(), " to VM #", getId(),
                        " failed by MIPS");
                getContainerRamProvisioner().deallocateRamForContainer(container);
            getContainerBwProvisioner().deallocateBwForContainer(container);
