...
public class ContainerDatacenter extends SimEntity {
    ...
    private List<Container> containerList1=new ArrayList<Container>();
    private List<Container> containerList2=new ArrayList<Container>();
     ...
     * @throws Exception
     */
    public ContainerDatacenter(String name, ContainerDatacenterCharacteristics characteristics,
    ...
    protected void processCloudletSubmit(SimEvent ev, boolean ack) {
        System.out.println("VMmigration-1-ContainerDatacenter.processCloudletSubmit");

        //updateCloudletProcessing_ContainerMigration();
    ...
    public void processEvent(SimEvent ev) {
        ...
        switch (ev.getTag()) {
            ...
            case CloudSimTags.VM_CREATE:
                System.out.println("未使用-processEvent");
                processVmCreate(ev, false);
                break;

            case CloudSimTags.VM_CREATE_ACK:
                System.out.println("VMallocation-0-0-1-PowerContainerDatacenterCM.processVmCreate");
            ...
            case CloudSimTags.VM_DATACENTER_EVENT:
                //updateCloudletProcessing_ContainerMigration();
                Log.printLine("VMmigration-1-ContainerDatacenter.processEvent");
                updateCloudletProcessing();
                checkCloudletCompletion();
                break;
     ...
     * @param V
     */
    private  void DynamicContainerAllocation(int n, List <Container> v, int V,ContainerVm vm) {
        ...
        int j=V;
        int sum=0;
        System.out.println("Vm:"+vm.getId()+" AvailableMips为:"+vm.getAvailableMips()+" 选中的容器是:");
        for (int i = n ; i > 0; i--){
            if (dpOne[i][j]>dpOne[i - 1][j]){
                ...
                System.out.print(containerList1.get(i-1).getId()+","+containerList1.get(i-1).getCurrentRequestedTotalMips()+" ");
                containerList1.remove(i-1);
            }
        }
    ...
    public void processContainerSubmit(SimEvent ev, boolean ack) {
        System.out.println("Containerallocation-1-ContainerDatacenter.processContainerSubmit");
        List<Container> containerList = (List<Container>) ev.getData();
        List<Container> containerList3=new ArrayList<Container>();
        List<Container> containerLista=new ArrayList<Container>();
        List<ContainerHost> containerHostListb=getHostList();
        containerList1.addAll(containerList);

        containerList2.addAll(containerList);
        containerLista.addAll(containerList);
        List<ContainerVm> containerVmList1=new ArrayList<ContainerVm>();
        containerVmList1.addAll(getContainerVmList());
        System.out.println(containerVmList1);
        PowerContainerList.sortByCurrentRequestedTotalMipsDecreasing(containerLista);
        PowerContainerVmList.sortByAvailableMipsDecreasing(containerVmList1);

        /**
         * 动态规划
         */
        for (int i = 0; containerList1.size()>0; i++) {
            ...
            //System.out.println("containerallocationcontainerList1大小:"+containerList1.size());
            DynamicContainerAllocation(containerList1.size(),containerList1,(int) containerVmList1.get(i).getAvailableMips(),containerVmList1.get(i));
        }
        for(int i=0;i<result1.size();i++){
            for (int j = 0; j <result1.get(i).size() ; j++) {
                containerList3.add(containerList2.get(result1.get(i).get(j)));
                containerList2.remove((int)result1.get(i).get(j));
            }
        }
        System.out.print("containerList3序列");
        for (Container container:containerList3){
         ...
         * 降序首次适应算法
         */
        //调用Containerallocation-2-PowerContainerList.sortByCurrentRequestedTotalMipsDecreasing
        PowerContainerList.sortByCurrentRequestedTotalMipsDecreasing(containerLista);
        //for (Container container : containerList) {
        //containerList3:动态规划；containerLista：首次降序适应

        for (Container container : containerList3) {
            /**
             * @see org.cloudbus.cloudsim.container.resourceAllocators.PowerContainerAllocationPolicy#allocateVmForContainer
             */
            //ContainerDatacenterBroker.containerVmList3（动态规划的vmList）,
            boolean result = getContainerAllocationPolicy().allocateVmForContainer(container, containerVmList1);
            if (ack) {
                int[] data = new int[3];
                ...
                if (result) {
                    System.out.println("Containerallocation-2-ContainerDatacenter.processContainerSubmit");
                    //调用Containerallocation-2-1-PowerContainerAllocationPolicy.getContainerVm
                    ContainerVm containerVm = getContainerAllocationPolicy().getContainerVm(container);
                    data[0] = containerVm.getId();
                    Log.printConcatLine("(DATACENTER): ",
                            "Assigning the container#" + container.getUid() + "to VM #" + containerVm.getUid());
        ...
        containerList1.clear();
        result1.clear();
        containerList2.clear();
    ...
    protected void processVmCreate(SimEvent ev, boolean ack) {
        ...
        System.out.println("VMallocation-1-ContainerDatacenter.processVmCreate");
        /**
         * @see org.cloudbus.cloudsim.container.resourceAllocators.PowerContainerVmAllocationAbstract#allocateHostForVm(ContainerVm)
         */
        boolean result = getVmAllocationPolicy().allocateHostForVm(containerVm);

        if (ack) {
            //System.out.println("VMallocation-1-1-");
            int[] data = new int[3];
            data[0] = getId();
    ...
    protected void processVmMigrate(SimEvent ev, boolean ack) {
        System.out.println("VMmigration-8-1-ContainerDatacenter.processVmMigrate");
        ...
        Log.formatLine("%.2f: VM #%d is removed from Host #%d", CloudSim.clock(), containerVm.getId(),
                host.getId());

        /**
         * @see org.cloudbus.cloudsim.container.resourceAllocators.PowerContainerVmAllocationAbstract#allocateHostForVm(ContainerVm, ContainerHost)
         */
    ...
    protected void processContainerMigrate(SimEvent ev, boolean ack) {
        ...
        System.out.println("CONTAINERmigration未知");
        Object tmp = ev.getData();
        if (!(tmp instanceof Map<?, ?>)) {
            throw new ClassCastException("The data object must be Map<String, Object>");
        }
    ...
    protected void processCloudletMove(int[] receivedData, int type) {
        System.out.println("未使用processCloudletMove");
        updateCloudletProcessing();
        ...
        if (cl == null) {// cloudlet doesn't exist
            ...
            // exist?
            if (destId == getId()) {
                ContainerVm containerVm = getVmAllocationPolicy().getHost(vmDestId, userId).getContainerVm(vmDestId,
                        userId);
                if (containerVm == null) {
                    failed = true;
                } else {
    ...
    protected void updateCloudletProcessing() {
        ...
        System.out.println("未使用updateCloudletProcessing");
     ...
     *            the new vm list
     */
    public <T extends ContainerVm> void setContainerVmList(List<T> containerVmList) {
    ...
    /**
     * 自己测试updateCloudletProcessing_ContainerMigration
     */
