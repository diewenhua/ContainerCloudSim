...
public abstract class PowerContainerVmAllocationPolicyMigrationAbstractContainerHostSelection extends PowerContainerVmAllocationPolicyMigrationAbstractContainerAdded {
    ...
    public Map<String, Object> findHostForContainer(Container container,List<ContainerHost> containerHostList, Set<? extends ContainerHost> excludedHosts, boolean checkForVM) {
        Log.printLine("ContainerMigration-8-PowerContainerVmAllocationPolicyMigrationAbstractContainerHostSelection.findHostForContainer");
        PowerContainerHost allocatedHost = null;
        ContainerVm allocatedVm = null;
        Map<String, Object> map = new HashMap<>();
        Set<ContainerHost> excludedHost1 = new HashSet<>();
        //excludedHosts:使用率为0的主机+overUtilizedHosts
        ...
        while (true) {
            ...
            /**
             * @see org.cloudbus.cloudsim.container.hostSelectionPolicies.HostSelectionPolicyFirstFit#getHost
             */
            ...
            for (int i = 0; i < vmList.size(); i++) {
                ...
                System.out.println("ContainerMigration-8-a");
                if (vm.isSuitableForContainer(container)) {
                    ...
                    System.out.println("ContainerMigration-8-b");
                    if (!isVmOverUtilized(vm)) {
                        continue;
                    }
                    System.out.println("ContainerMigration-8-c");
    ...
    public Map<String, Object> findVmForContainer1(Container container,List<ContainerVm> vmList1, Set<? extends ContainerHost> excludedHosts, boolean checkForVM) {
        Log.printLine("ContainerMigration-8-PowerContainerVmAllocationPolicyMigrationAbstractContainerHostSelection.findVmForContainer1");
        PowerContainerHost allocatedHost = null;
        ContainerVm allocatedVm = null;
        Map<String, Object> map = new HashMap<>();
//        Set<ContainerHost> excludedHost1 = new HashSet<>();
...
//                return map;
//            }
            //ContainerHost host = getHostSelectionPolicy().getHost(getContainerHostList(), container, excludedHosts);
            boolean findVm = false;
            //List<ContainerVm> vmList = host.getVmList();
            //PowerContainerVmList.sortByCpuUtilization(vmList);
            //for (int i = 0; i < vmList1.size(); i++) {
            for (ContainerVm vm :vmList1){
                if(checkForVM){
                    if(vm.isInWaiting()){
                        continue;
                    }
                ...
                System.out.println("findVmForContainer1-8-a");
                if (vm.isSuitableForContainer(container)) {
                    // if vm is overutilized or host would be overutilized after the allocation, this host is not chosen!
                    //System.out.println("ContainerMigration-8-b");
                    if (!isVmOverUtilized(vm)) {
                        System.out.println("ContainerMigrationVM:"+vm.getId()+" isOverUtilized");
                        continue;
                    }
                    //System.out.println("ContainerMigration-8-c");
                    if (getUtilizationOfCpuMips((PowerContainerHost) host) != 0 && isHostOverUtilizedAfterContainerAllocation((PowerContainerHost) host, vm, container)) {
                        System.out.println("ContainerMigrationHostIs:"+vm.getId()+" isOverUtilized"+" vm"+vm.getId()+" container"+container.getId());
                        continue;
                    }
                    System.out.println("ContainerMigrationVM:"+vm.getId()+",AvailableMIPS:"+vm.getAvailableMips()+" Container:"+container.getId()+",CurrentRequestedTotalMips:"+container.getCurrentRequestedTotalMips());
                    vm.containerCreate(container);
                    System.out.println("ContainerMigrationVMA:"+vm.getId()+"分配之后的AvailableMIPS:"+vm.getAvailableMips());
                    allocatedVm = vm;
                    findVm = true;
                    allocatedHost = (PowerContainerHost) host;
                    break;
                }else{
                    System.out.println("ContainerMigrationVM:"+vm.getId()+" isNotSuitableForContainer:"+container.getId());
                }
            }
            if (findVm) {
                map.put("vm", allocatedVm);
...
//    protected Collection<? extends Map<String, Object>> getContainerMigrationMapFromUnderUtilizedHostsC(List<PowerContainerHostUtilizationHistory> overUtilizedHosts, List<Map<String, Object>> previouseMap) {
//        Log.printLine("ContainerMigration-4-PowerContainerVmAllocationPolicyMigrationAbstractContainerHostSelection.getContainerMigrationMapFromUnderUtilizedHosts");
//        List<Map<String, Object>> migrationMap = new LinkedList<Map<String, Object>>();
//        /**
//         * @see org.cloudbus.cloudsim.container.resourceAllocatorMigrationEnabled.PowerContainerVmAllocationPolicyMigrationAbstract#getSwitchedOffHosts
//         */
//        List<PowerContainerHost> switchedOffHosts = getSwitchedOffHosts();
//        List<Container> containersToMigrateFromUnderUtilizedHost=new ArrayList<Container>();
//
//        // over-utilized hosts + hosts that are selected to migrate VMs to from over-utilized hosts
//        Set<PowerContainerHost> excludedHostsForFindingUnderUtilizedHost = new HashSet<>();
//        excludedHostsForFindingUnderUtilizedHost.addAll(overUtilizedHosts);
//        excludedHostsForFindingUnderUtilizedHost.addAll(switchedOffHosts);
//        //从previouseMap中提取Host
...
//        }
//
//        for (PowerContainerHost underUtilizedHost1:getUnderUtilizedHostsC(excludedHostsForFindingUnderUtilizedHost)){
//            Log.printConcatLine("Under-utilized host: host #", underUtilizedHost.getId(), "\n");
//            containersToMigrateFromUnderUtilizedHost.addAll(getContainersToMigrateFromUnderUtilizedHost(underUtilizedHost1));
//        }
...
//
//        //调用同类getContainersToMigrateFromUnderUtilizedHost
...
//                Add the host to the placement founder option
    ...
    protected List<? extends Container> getContainersToMigrateFromUnderUtilizedHost(PowerContainerHost host) {
        Log.printLine("ContainerMigration-6-PowerContainerVmAllocationPolicyMigrationAbstractContainerHostSelection.getContainersToMigrateFromUnderUtilizedHost");
    ...
    protected List<Map<String, Object>> getNewContainerPlacementFromUnderUtilizedHost(
        ...
        Log.printLine("ContainerMigration-7-PowerContainerVmAllocationPolicyMigrationAbstractContainerHostSelection.getNewContainerPlacementFromUnderUtilizedHost");
        List<Map<String, Object>> migrationMap = new LinkedList<Map<String, Object>>();
        //调用Containerallocation-2-0-PowerContainerList.sortByCpuUtilization
        PowerContainerList.sortByCpuUtilization(containersToMigrate);
        for (Container container : containersToMigrate) {
            //
            Map<String, Object> allocatedMap = findHostForContainer(container,getContainerHostList(), excludedHosts, true);
