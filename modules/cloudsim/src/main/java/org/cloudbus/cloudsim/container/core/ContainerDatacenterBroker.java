...
public class ContainerDatacenterBroker extends SimEntity {
    ...
    protected List<? extends ContainerHost> hostList;
...
/**
 ...
 /**
    ...
    private List<ContainerVm> containerVmList1=new ArrayList<ContainerVm>();
    private List<ContainerVm> containerVmList2=new ArrayList<ContainerVm>();
    public static List<ContainerVm> containerVmList3=new ArrayList<ContainerVm>();
    private List<List<Integer>> result1 = new ArrayList<List<Integer>>();

    public static List<ContainerHost> containerHostList111=new ArrayList<ContainerHost>();
    ...
    public ContainerDatacenterBroker(String name, double overBookingfactor) throws Exception {
        ...
        setHostList(new ArrayList<ContainerHost>());
    ...
    public void setHostList(List<? extends ContainerHost> List) {
        this.hostList = List;
    }
    ...
    public void processEvent(SimEvent ev) {
        switch (ev.getTag()) {
            ...
            case CloudSimTags.VM_CREATE_ACK:
                System.out.println("VMallocation-10-0-ContainerDatacenterBroker.processEvent");
                //调用本类VMallocation-6-ContainerDatacenterBroker.processVmCreate
                processVmCreate(ev);
                break;
...
//            ContainerVm p= ContainerVmList.getById(getVmsCreatedList(), vmId);
            ...
            //Container container = ContainerList.getById(getContainerList(), contaCannot find local variable 'hostId'inerId);
        ...
        if (getContainersAcks() == getContainerList().size()) {
            ...
            //System.out.println("调用cloudlet-2-");
            submitCloudlets();
...
//                getDatacenterIdsList().size(), " resource(s)");
    ...
    protected void processVmCreate(SimEvent ev) {
        System.out.println("VMallocation-10-ContainerDatacenterBroker.processVmCreate");
        int[] data = (int[]) ev.getData();
...
//        }
    ...
    private  void DynamicVmAllacation1(int n, List <ContainerVm> v, int V,ContainerHost host) {
        //System.out.println("物体个数："+v.size());
        int [][] dpOne =new int [n+1][V+1];
        for (int i = 1; i <= n; i++) {
            //int z=(int)((v.get(i-1).getCurrentRequestedTotalMips()));
            int z=(int)(Math.ceil((v.get(i-1).getCurrentRequestedTotalMips())));
            for (int j = 1; j <= V; j++) {
                //if (j >= (int)(Math.ceil((v.get(i-1).getCurrentRequestedTotalMips())))) {
                if (j >= z) {
        ...
        int j=V;
        int sum=0;
        System.out.println("DynamicVmAllacation1Host:"+host.getId()+" AvailableMips为:"+host.getAvailableMips()+"  选中的虚拟机是:");
        for (int i = n ; i > 0; i--){
            if (dpOne[i][j]>dpOne[i - 1][j])
            {
                x.add(i-1);
                sum+=(int)(Math.ceil((v.get(i-1).getCurrentRequestedTotalMips())));
                //j = j - (int)((v.get(i-1).getCurrentRequestedTotalMips()));
                j = j - (int)(Math.ceil((v.get(i-1).getCurrentRequestedTotalMips())));
                System.out.print(containerVmList1.get(i-1).getId()+","+containerVmList1.get(i-1).getCurrentRequestedTotalMips()+" ");
                containerVmList1.remove(i-1);
            }
        }
        System.out.println();
        System.out.println("虚拟机RequestedTotalMips总和:"+sum);
        result1.add(x);
...
//		}
    ...
    protected void createVmsInDatacenter(int datacenterId) {
        System.out.println("VMallocation-0-0-0-ContainerDatacenterBroker.createVmsInDatacenter");
        // send as much vms as possible for this datacenter before trying the next one
        int requestedVms = 0;
        String datacenterName = CloudSim.getEntityName(datacenterId);

        List<ContainerVm> containerVmList = getVmList();
        List<ContainerVm> containerVmListc=new ArrayList<ContainerVm>();
        containerVmList1.addAll(containerVmList);
        containerVmList2.addAll(containerVmList);
        containerVmListc.addAll(containerVmList);
        containerHostList111.addAll(this.hostList);
        ContainerHostList.sortByAvailableMipsDescending(containerHostList111);

        PowerContainerVmList.sortByCurrentRequestedTotalMipsDecreasing(containerVmListc);
        /**
         * 动态规划
         */
        System.out.println("PowerContainerVmAllocationPolicyMigrationStaticThresholdMC.overUtilizationThreshold:"+PowerContainerVmAllocationPolicyMigrationStaticThresholdMC.overUtilizationThreshold);
        for (int i = 0; containerVmList1.size()>0; i++) {
            //DynamicVmAllacation1(containerVmList1.size(),containerVmList1,(int) (containerHostList111.get(i).getAvailableMips()*PowerContainerVmAllocationPolicyMigrationStaticThresholdMC.overUtilizationThreshold),containerHostList111.get(i));
            DynamicVmAllacation1(containerVmList1.size(),containerVmList1,(int) (containerHostList111.get(i).getAvailableMips()),containerHostList111.get(i));
        }
        for(int i=0;i<result1.size();i++){
            for (int j = 0; j <result1.get(i).size() ; j++) {
                containerVmList3.add(containerVmList2.get(result1.get(i).get(j)));
                containerVmList2.remove((int)result1.get(i).get(j));
            }
        }
        System.out.print("containerVmList3序列");
        for (ContainerVm vm:containerVmList3){
            System.out.print(vm.getId()+",");
        }
        System.out.println();
//        System.out.println("containerVmList3大小："+containerVmList3.size()+"containerVmList1大小："+
//                containerVmList1.size()+"containerVmList2大小："+containerVmList2.size());

        /**
         * 首次降序适应算法
         */
        //containerVmList3动态规划，containerVmListc首次降序适应
        for (ContainerVm vm : containerVmList3) {
        ...
        setVmsAcks(0);
        containerVmList1.clear();
        containerVmList2.clear();
        containerVmList3.clear();
        result1.clear();
    }
        ...
        for(Container container:getContainerList()) {
            ContainerCloudlet cloudlet = getCloudletList().get(i);
                ...
                if (cloudlet.getUtilizationModelCpu() instanceof UtilizationModelPlanetLabInMemory) {
                    ...
                    //Log.printLine("Container Index" + containerIndex);
                    System.out.print("cloudletUsage:");
                    double newmips = percentileUsage * container.getMips();
                    for (int j = 0; j < cloudletUsage.length; j++) {
                        System.out.print(cloudletUsage[j]+" ");
                    }
                    System.out.println();
                    System.out.println("cloudletUsage(Average):"+Arrays.stream(cloudletUsage).average().orElse(Double.NaN)+
                            " percentileUsage:"+percentileUsage+" newmips:"+newmips);
