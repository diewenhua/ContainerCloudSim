...
public class ContainerHost {
    ...
    public double updateContainerVmsProcessing(double currentTime) {
        System.out.println("VMmigration-5-ContainerHost.updateContainerVmsProcessing");
    ...
    public void addMigratingInContainerVm(ContainerVm containerVm) {
        ...
        System.out.println("HostPEB-未使用-1-ContainerHost.containerVmCreate");
    ...
    public void reallocateMigratingInContainerVms() {
        System.out.println("HostPEC-1-ContainerHost.containerVmCreate");
    ...
    public boolean isSuitableForContainerVm(ContainerVm vm) {
        ...
        System.out.println("VMallocation-5-1-ContainerHost.isSuitableForContainerVm");
        double currentRequestedMaxMips1=vm.getCurrentRequestedMaxMips();
        double currentRequestedTotalMips1=vm.getCurrentRequestedTotalMips();
        List<Double> currentRequestedMips1 =new ArrayList<Double>();
        currentRequestedMips1.addAll(vm.getCurrentRequestedMips());
//        System.out.println("vm.getCurrentRequestedTotalMips:"+currentRequestedTotalMips1+" "+currentRequestedMips1+" "+vm.getMips()+"*"+vm.getNumberOfPes());
//        System.out.println("PeCapacity和currentRequestedMaxMips:"+getContainerVmScheduler().getPeCapacity()+" "+currentRequestedMaxMips1);
        //getCurrentRequestedMaxMips:VMallocation-5-2-ContainerVm.getCurrentRequestedMaxMips
        //getCurrentRequestedTotalMips:VMallocation-5-3-ContainerVm.getCurrentRequestedTotalMips
        //getPeCapacity:单个PE的能力    getCurrentRequestedMaxMips:取getCurrentRequestedMips（队列）中最大的元素
        //getContainerVmScheduler().getAvailableMips():Host剩余总的MIPS
...
//                .isSuitableForContainerVm(vm, vm.getCurrentRequestedBw()));
        ...
        if(!getContainerVmRamProvisioner().isSuitableForContainerVm(vm, vm.getCurrentRequestedRam())){
            System.out.println("host:"+this.getId()+"RamisNotSuitableForContainerVm:"+vm.getId());
        }
        if(!getContainerVmBwProvisioner().isSuitableForContainerVm(vm, vm.getCurrentRequestedBw())){
            System.out.println("host:"+this.getId()+"BwisNotSuitableForContainerVm:"+vm.getId());
        }
        return (getContainerVmScheduler().getAvailableMips() >= currentRequestedTotalMips1
                && getContainerVmRamProvisioner().isSuitableForContainerVm(vm, vm.getCurrentRequestedRam()) && getContainerVmBwProvisioner()
                .isSuitableForContainerVm(vm, vm.getCurrentRequestedBw()));
    }
    ...
    public boolean containerVmCreate(ContainerVm vm) {
        ...
        System.out.println("HostPEA-1-ContainerHost.containerVmCreate");
        ...
        /**
         * @see org.cloudbus.cloudsim.container.schedulers.ContainerVmSchedulerTimeShared#allocatePesForVm
         */
        //vm.getCurrentRequestedMips()为List<Double>类型
        List<Double> currentRequestedMips1 = new ArrayList<>(getNumberOfPes());
        currentRequestedMips1.addAll(vm.getCurrentRequestedMips());
        //if (!getContainerVmScheduler().allocatePesForVm(vm, vm.getCurrentRequestedMips())) {
        if (!getContainerVmScheduler().allocatePesForVm(vm, currentRequestedMips1)) {
            Log.printConcatLine("[VmScheduler.containerVmCreate] Allocation of VM #", vm.getId(), " to Host #", getId(),
                    " failed by MIPS");
            getContainerVmRamProvisioner().deallocateRamForContainerVm(vm);
    ...
    public double getTotalAllocatedMipsForContainerVm(ContainerVm containerVm) {
        ...
        //System.out.println("VMallocation-5A-1-ContainerHost.getTotalAllocatedMipsForContainerVm");
