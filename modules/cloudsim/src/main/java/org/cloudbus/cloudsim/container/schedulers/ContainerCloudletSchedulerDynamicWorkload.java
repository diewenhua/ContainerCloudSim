...
public class ContainerCloudletSchedulerDynamicWorkload extends ContainerCloudletSchedulerTimeShared {
        ...
        public double getTotalUtilizationOfCpu(double time) {
            //System.out.println("Containerallocation-2-0-3-ContainerCloudletSchedulerDynamicWorkload.getTotalUtilizationOfCpu");
            double totalUtilization = 0;
            for (ResCloudlet rcl : getCloudletExecList()) {
                /**
                 * @see org.cloudbus.cloudsim.Cloudlet#getUtilizationOfCpu
                 */
        ...
        public List<Double> getCurrentRequestedMips() {
            //System.out.println("VMallocation-5-3-3-ContainerCloudletSchedulerDynamicWorkload.getCurrentRequestedMips");
            if (getCachePreviousTime() == getPreviousTime()) {
                //System.out.println("VMallocation-5-3-3-A");
                return getCacheCurrentRequestedMips();
            }
            //System.out.println("VMallocation-5-3-3-B");
            List<Double> currentMips = new ArrayList<>();
            //调用同类getTotalUtilizationOfCpu
            double totalMips = getTotalUtilizationOfCpu(getPreviousTime()) * getTotalMips();
            //System.out.println("totalMips:"+totalMips+" "+getNumberOfPes()+" "+getTotalMips());
